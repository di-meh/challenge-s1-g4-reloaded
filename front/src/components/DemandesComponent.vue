...
const acceptAnnonceur = async () => {
    const userId = props.userId.split("/").pop();
    const demandeId = props.demandeId.split("/").pop();
    const response = await fetch(`${ENTRYPOINT}/users/${userId}/update_annonceur`, {
        method: "PATCH",
        headers: {
            "Content-Type": "application/merge-patch+json",
            Authorization: `Bearer ${token}`,
...
<template>
    <li class="col-span-1 divide-y divide-gray-200 rounded-lg bg-white shadow flex flex-col	 justify-between">
        <h3 class="truncate text-l font-medium text-black text-center p-2">Type : {{ type }}</h3>
        <div class="flex w-full items-center justify-between space-x-6 p-6">
            <div class="flex-1 truncate">
                <div class="flex items-center space-x-3">
                    <h3 class="truncate text-sm font-medium text-gray-900">Nom : {{ username }}</h3>
                </div>
                <p v-if="type == 'Annonceur'" class="mt-1 truncate text-sm text-gray-500">Entreprise : {{ entrepriseName }}</p>
                <p v-if="type == 'Annonceur'" class="mt-1 truncate text-sm text-gray-500">Lien : {{ entrepriseLink }}</p>
        ...
        <div>
            <div class="-mt-px flex divide-x divide-gray-200">
                <div class="flex w-0 flex-1">
                    <button v-if="type == 'Annonceur'" @click="acceptAnnonceur"
                    ...
                    <button v-if="type == 'Vendeur'" @click="acceptVendeur"
                        class="cursor-pointer text-green-600 relative -mr-px inline-flex w-0 flex-1 items-center justify-center gap-x-3 rounded-bl-lg border border-transparent py-4 text-sm font-semibold text-gray-900">
                        Accept
                    </button>
                </div>
